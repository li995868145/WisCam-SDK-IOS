...
@protocol WisViewDelegate <NSObject>
/*
state:
 STATE_IDLE = 0,
 STATE_PREPARING = 1,
...
- (void)take_imageRef:(CGImageRef)imageRef;//回调获取rgb555 le格式的imageRe
@end

@interface WisView : UIView
/*
 LOG_LEVEL_VERBOSE 0
 LOG_LEVEL_DEBUG 1
 LOG_LEVEL_INFO 2
...
- (void)hardwareDecoder:(BOOL)enable;//使能硬件解码
- (void)startGetH264Data:(BOOL)enable;//使能后即可通过回调获得h264数据
- (void)startGetYUVData:(BOOL)enable;//使能后即可通过回调获取解码后的YUV数据
- (void)startGetAudioData:(BOOL)enable;//使能后即可通过回调获取音频数据
- (void)show_view:(BOOL)enable;//是否播放显示视频
- (id)init;//初始化
...
- (void)delegate:(id<WisViewDelegate>)d;//代理
@end
